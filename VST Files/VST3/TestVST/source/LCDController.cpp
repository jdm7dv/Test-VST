...
//#include "Windows/DSPKit 2.4/pluginconstants.h"
...
#else
CView* LCDController::verifyView (CView* view, const UIAttributes& attributes, const IUIDescription* description)
#endif
{
	// --- (1) the value knob
	...
	if(edit)
	{
		...
		// NO if one object, one control listener
		// m_pControlEdit->setListener(this);
...
{
	if(pControl == m_pLCDValueKnob && m_pCurrentParameter)
	{
