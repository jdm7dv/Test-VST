...
{
    m_bDraggingPuck = false;
	return CXYPad::onMouseUp(where, buttons);
}
...
void CXYPadWP::draw(CDrawContext* context)
{
	...
	// --- now the axes(?)
	const VSTGUI::CCoord centerX = xyRect.left + xyRect.getWidth()/2.0;
	const VSTGUI::CCoord centerY = xyRect.top + xyRect.getHeight()/2.0;
